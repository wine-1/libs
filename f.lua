...
function Options:AddFloatingButton(Settings)
	-- Settings can include: Position, Icon, Size, etc.
	local floatButton = Instance.new("ImageButton")
	local floatFrame = Instance.new("Frame")
	local shadow = Instance.new("UIStroke")
	
	-- Default settings
	local position = Settings.Position or UDim2.new(0.5, 250, 0.5, 0) -- Right side of UI
	local icon = Settings.Icon or "rbxassetid://7072725342" -- Default hide icon
	local size = Settings.Size or UDim2.new(0, 40, 0, 40)
	local color = Settings.Color or Color3.fromRGB(45, 45, 45)
	local cornerRadius = Settings.CornerRadius or UDim.new(0, 8)
	
	-- Create the floating container
	SetProperty(floatFrame, {
	...
	-- Add drag functionality for the floating button
	local dragging
	local dragInput
	local dragStart
	local startPos
	...
	Connect(floatFrame.InputBegan, function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			dragging = true
	...
	Connect(floatFrame.InputChanged, function(input)
	...
	Connect(floatButton.MouseButton1Click, function()
		if isUIVisible then
			...
			floatButton.Image = Settings.UnhideIcon or "rbxassetid://7072725676"
			
			-- Optional: Shrink the floating button
			Tween(floatFrame, 0.2, {
				Size = size - UDim2.new(0, 10, 0, 10),
	...
	-- Add hover effects
	Connect(floatButton.MouseEnter, function()
		Tween(floatButton, 0.2, { ImageTransparency = 0 })
		Tween(floatFrame, 0.2, { 
	...
	Connect(floatButton.MouseLeave, function()
		Tween(floatButton, 0.2, { ImageTransparency = 0.1 })
		Tween(floatFrame, 0.2, { 
			BackgroundTransparency = 0.1,
			Size = size
		})
	...
	Options.FloatingButton = floatFrame
	
	return {
		Button = floatButton,
		Container = floatFrame,
		Toggle = function()
			floatButton.MouseButton1Click()
		end,
		SetPosition = function(newPosition)
			floatFrame.Position = newPosition
		end,
		SetIcon = function(newIcon)
			floatButton.Image = newIcon
		end,
		Destroy = function()
			floatFrame:Destroy()
		end
