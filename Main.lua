...
-- added: small persistent toggle button to open/close all created windows
do
	local ToggleButton = Instance.new("TextButton")
	ToggleButton.Name = "UI_Toggle"
	ToggleButton.AnchorPoint = Vector2.new(0, 1)
	ToggleButton.Position = UDim2.new(0, 10, 1, -10) -- bottom-left with margin
	ToggleButton.Size = UDim2.new(0, 120, 0, 36)
	ToggleButton.BackgroundColor3 = Theme.Interactables
	ToggleButton.TextColor3 = Theme.Title
	...
	ToggleButton.MouseButton1Click:Connect(function()
		for _, ToggleFunc in next, Library.Windows do
			pcall(ToggleFunc)
		end
	end)
end
...
-- changed: initialize a place to store created windows/toggles
...
function Library:CreateWindow(Settings: { Title: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string })
	...
	-- store the toggle/close function so the persistent Toggle button can toggle this window
	Library.Windows[Settings.Title] = Close
